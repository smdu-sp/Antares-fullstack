...
/** Porte de AndamentosService.contarEmAndamento (Antares-backend/src/andamentos/andamentos.service.ts). */
export async function contarEmAndamento(usuario_id?: string): Promise<number> {
  ...
  const searchParams: Prisma.andamentoWhereInput = {
    ...
    OR: [{ prazo: { gte: hoje }, prorrogacao: null }, { prorrogacao: { gte: hoje } }],
  };
