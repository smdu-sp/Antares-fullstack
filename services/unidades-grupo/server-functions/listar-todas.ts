...
/** Espelha services/unidades/server-functions/listar-todas.ts, pra unidades-grupo (por grupo). */
export async function listarTodas(): Promise<IUnidade[]> {
  ...
  try {
    const response = await fetch(`${baseURL}unidades-grupo/lista-completa`, {
      method: "GET",
...
export async function reativarUnidade(
  ...
  try {
    const response = await fetch(`${baseURL}unidades-grupo/${id}/reativar`, {
      method: "PATCH",
      headers: buildAuthHeaders(session.access_token, session.grupoAtivo?.id),
